...
"""
Listing 创建流程编排
InventoryItem → Offer → (可选) Publish

单一职责：将一个商品数据 dict 转化为 eBay 上的 Listing
"""
...
class ListingCreator:
    ...
    def create_listing(self, item: dict, auto_publish: bool = False) -> ListingResult:
        ...
        Args:
            item: 商品数据 dict，包含以下字段:
                - sku, title, description, category_id, price, quantity
                - image_urls (逗号分隔), condition, brand, mpn, upc
                - weight_kg, length_cm, width_cm, height_cm
            publish: 是否发布（默认 False，只创建草稿）
        ...
        if not result.offer_id:
            logger.warning(f"[{sku}] API 未返回 offer_id（沙盒已知问题）, 尝试通过查询获取...")
        ...
        if auto_publish and result.offer_id:
            ...
            logger.info(f"[{sku}] 已创建为草稿 (UNPUBLISHED), 可在 Seller Hub Drafts 中查看")
    ...
    def _create_inventory_item(self, item: dict):
        """创建/更新 InventoryItem（增强版：支持更多商品属性）"""
        sku = item["sku"]
        ...
        # 处理 UPC，注意 pandas 可能返回 float('nan')
        upc = item.get("upc", "")
    ...
    def _get_condition(self, item: dict) -> str:
        """获取 condition enum (InventoryItem API 需要 enum 值，不是 ID)"""
        cond = str(item.get("condition", "NEW")).strip().upper()
        # InventoryItem API 需要 enum 值如 "NEW", "LIKE_NEW" 等
        # 而不是 conditionId 如 "1000", "3000" 等
