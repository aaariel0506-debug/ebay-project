#!/usr/bin/env python3
"""
生成 2026 年 2 月 eBay 报税报表 - 修复版 v3
"""
import sqlite3
import csv
...
def import_ebay():
    ...
    for i, col in enumerate(header):
        col_lower = col.lower()
        if 'order number' in col_lower:
            col_map['order'] = i
        elif 'sale date' in col_lower:
            col_map['date'] = i
        elif 'sold for' in col_lower:
            col_map['sold_for'] = i
        elif 'shipping' in col_lower and 'handling' in col_lower:
            col_map['shipping'] = i
        elif 'tracking' in col_lower:
            col_map['tracking'] = i
        elif 'ship to country' in col_lower:
            col_map['country'] = i
        elif 'item title' in col_lower:
            col_map['title'] = i
        elif 'item number' in col_lower:
            col_map['item_id'] = i
        elif 'quantity' in col_lower:
            col_map['qty'] = i
        elif 'buyer username' in col_lower:
            col_map['buyer'] = i
    ...
    for line in lines[3:]:
        try:
            ...
            order_num = row[col_map.get('order', 1)].strip()
            if not order_num:
                continue
            
            # 解析日期
            date_idx = col_map.get('date', 49)
            sale_date = parse_date_us(row[date_idx]) if date_idx < len(row) else None
            
            if sale_date and sale_date.startswith('2026-02'):
                feb += 1
            
            # 解析金额
            sold_for = parse_money(row[col_map.get('sold_for', 26)]) if col_map.get('sold_for', 26) < len(row) else 0
            shipping = parse_money(row[col_map.get('shipping', 27)]) if col_map.get('shipping', 27) < len(row) else 0
...
def import_amazon():
    ...
    for line in lines:
        try:
            ...
            qty = int(row[3]) if len(row) > 3 and row[3].strip() else 1
            total = parse_money(row[6]) if len(row) > 6 else 0
            tax = parse_money(row[8]) if len(row) > 8 else 0
            
            c.execute("INSERT OR REPLACE INTO purchases VALUES (?,?,?,?,?,?,?,?,?)",
                (pid, 'amazon_jp', purchase_date, row[53] if len(row) > 53 else '',
                 asin, qty, total, tax, order_num))
            imported += 1
        except Exception as e:
            pass
