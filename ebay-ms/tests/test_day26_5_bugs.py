...
Day 26.5 代码审查发现的 4 个未修复 bug,每个对应一个 xfail(strict=True) 测试。
UM870 修好后 xfail 会变 xpass → strict=True 报错,必须移除 @xfail 装饰器。

Bug 1 (order_sync_service):多 SKU 订单的 FEE/SHIPPING Transaction 被按 SKU 重复写
Bug 2 (alembic migration):upgrade 后 orders 表失去 PRIMARY KEY 等约束
Bug 3 (alembic migration):downgrade 销毁 orders 的订单级字段数据
Bug 4 (OrderItem schema):__table_args__ 注释声称有 UNIQUE 约束但实际是空 tuple
"""
...
@pytest.mark.xfail(
    reason=(
        "Day 26.5 Bug 1 待修复:order_sync_service._upsert_order 在 line_item "
        "循环里调用 _write_transactions 时传入订单级 shipping_cost 和 "
        "total_fee,_write_transactions 按 (order_id, sku, type) 去重,导致"
        "多 SKU 订单的 FEE/SHIPPING 被按 SKU 重复写 N 倍。修复方向:把 "
        "FEE/SHIPPING 写入从 _write_transactions 剥离,per-order 写一次,"
        "Transaction.sku 填 NULL(订单级流水)。"
    ),
    strict=True,
)
...
@pytest.mark.xfail(
    reason=(
        "Day 26.5 Bug 2 待修复:alembic/versions/82b6ba3706c4 的 upgrade() "
        "用 CREATE TABLE orders_new AS SELECT ... 重建 orders 表,这种语法"
        "只复制数据不复制约束,orders 表丢失 PRIMARY KEY、NOT NULL、列类型"
        "精度。修复方向:用 op.create_table(..., sa.Column(..., primary_key=True))"
        "显式声明,或用 batch_alter_table + drop_column。"
    ),
    strict=True,
)
...
@pytest.mark.xfail(
    reason=(
        "Day 26.5 Bug 3 待修复:alembic/versions/82b6ba3706c4 的 downgrade() "
        "第 118-150 行顺序错误:DROP orders → CREATE 空 orders → UPDATE 空表 → "
        "DROP orders_old 备份。导致 downgrade 后 orders 表的订单级字段"
        "(shipping_cost/ebay_fee/buyer_country/status/dates)全部为 NULL。"
        "修复方向:从 orders_old INSERT INTO orders SELECT *,再 UPDATE sku。"
    ),
    strict=True,
)
def test_bug3_downgrade_preserves_order_level_fields(tmp_path):
    ...
    # products 先建(FK 要)
    cur.execute("""
        INSERT INTO products (sku, title, status, cost_currency, created_at, updated_at)
        VALUES ('SKU-BUG3', 'Bug3 Product', 'active', 'USD',
    ...
    assert r.returncode == 0, f"downgrade -1 失败:\nstdout={r.stdout}\nstderr={r.stderr}"
...
@pytest.mark.xfail(
    reason=(
        "Day 26.5 Bug 4 待修复:core/models/order.py 的 OrderItem.__table_args__ "
        "写了三行注释说 '同一 order_id + sku 应唯一',但实际是空 tuple。"
        "修复方向:UniqueConstraint('order_id', 'sku', name='uq_order_items_order_sku')"
        ",并在 migration 里加对应的 op.create_unique_constraint。"
    ),
    strict=True,
)
